...
type Node struct {
	...
	stopCh             chan struct{}
	crashed            bool
	networkPartitioned map[int]bool // NEW: network partition simulation
}
...
func main() {
	...
	flex := tview.NewFlex().SetDirection(tview.FlexRow).
		...
		AddItem(tview.NewTextView().
			SetText("Press 0-4 to toggle node crash/recover | Press 'pXY' to partition X and Y | Ctrl+C to quit").
